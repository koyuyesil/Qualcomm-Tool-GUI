...
namespace Qualcomm_Tools_GUI
{
    ...
    public partial class Main
    {
        ...
        public int tot = 0;
        public string MSM_HW_ID = "";
        public string OEM_PK_HASH = "";
        ...
        private void btngpt_Click(object sender, EventArgs e)
        {
            ...
            if (CheckBoxAutoLoader.Checked)
            {
                flags = true;
            }
            else if (string.IsNullOrEmpty(txtloader.Text))
            {
                ...
                while (num1 < strArrays.Length)
                {
                    ...
                    if (Operators.CompareString(str1, "", false) != 0)
                    {
                        ...
                        if (CheckBoxAutoLoader.Checked)
                        {
                            ...
                            string folder = Application.StartupPath + @"\Data\Autoloader";
                            string string_to_find = MSM_HW_ID + "_" + OEM_PK_HASH;

                            var di = new DirectoryInfo(folder);

                            var results = di.EnumerateFileSystemInfos("*", SearchOption.AllDirectories).Where(i => i.Name.IndexOf(string_to_find, StringComparison.InvariantCultureIgnoreCase) >= 0);
                            do
                            {
                                bool exitDo = false;
                                foreach (var r in results)
                                {
                                    ...
                                    {
                                        txtloader.Invoke(new Action(() => txtloader.Text = r.FullName));
                                        exitDo = true;
                                        break;
                                    }
                                }
        ...
        private void CheckBoxAutoLoader_CheckedChanged(object sender, EventArgs e)
        {
            if (CheckBoxAutoLoader.Checked)
            {
                txtloader.Text = "";
